...
class Node(object):
    ...
    def ucb(self, rollout_mode):
        return _mcts.Node_ucb(self, rollout_mode)
...
def select(root, rollout_mode):
    return _mcts.select(root, rollout_mode)

def expand(selected_node):
    return _mcts.expand(selected_node)

def rollout(selected_node, rollout_mode):
    return _mcts.rollout(selected_node, rollout_mode)

def backpropagate(selected_node):
    return _mcts.backpropagate(selected_node)

def select_best_action(root):
    return _mcts.select_best_action(root)

def run(level, timeout_ms, games_per_rollout, debug=False, ucb_const=1.4, rollout_mode=0):
    return _mcts.run(level, timeout_ms, games_per_rollout, debug, ucb_const, rollout_mode)

def parralel_run(level, timeout_ms, parralel_factor, debug, ucb_const):
    return _mcts.parralel_run(level, timeout_ms, parralel_factor, debug, ucb_const)
